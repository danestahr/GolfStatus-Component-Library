...
const GSInfoGroup = props => {
  ...
  const getTag = sectionItem => {
    ...
    if (type.includes?.("headline-1")) {
      return (
        <h1
          style={{ ...defaultTypography.headline1, ...sectionItemStyle }}
          className={sectionItem.type}
        >
          {sectionItem.value}
        </h1>
      );
    ...
    if (type.includes?.("headline-2")) {
      return (
        <h2
          style={{ ...defaultTypography.headline2, ...sectionItemStyle }}
          className={sectionItem.type}
        >
          {sectionItem.value}
        </h2>
    ...
    if (type.includes?.("headline-3")) {
      return (
        <h3
          style={{ ...defaultTypography.headline3, ...sectionItemStyle }}
          className={sectionItem.type}
        >
          {sectionItem.value}
        </h3>
    ...
    if (type.includes?.("headline-4")) {
      return (
        <h4
          style={{ ...defaultTypography.headline4, ...sectionItemStyle }}
          className={sectionItem.type}
        >
          {sectionItem.value}
        </h4>
    ...
    if (type.includes?.("headline-5")) {
      return (
        <h5
          style={{ ...defaultTypography.headline5, ...sectionItemStyle }}
          className={sectionItem.type}
        >
          {sectionItem.value}
        </h5>
  ...
  const getSection = section => {
    const { sectionItems, gap, sectionStyle, style } = section ?? {};
    return (
      <GSItemList
        style={sectionStyle}
        listStyle={style}
        type={`vertical ${gap ?? itemGap ?? ""} info-section ${section.type ??
          ""}`}
        items={sectionItems}
        listItem={sectionItem => getTag(sectionItem)}
      />
    );
  };
