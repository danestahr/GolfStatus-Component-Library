...
export default function GSObjectView(props) {
  const { obj, style, formStyle, detailsStyle, itemListStyle, listStyle, fieldStyle } = props;
        ...
        {obj && (
          <div className="details" style={detailsStyle}>
            <GSItemList
              ...
              listStyle={listStyle}
              type="vertical"
              items={displayableProperties()}
              listItem={item => (
                <GSField
                  style={fieldStyle}
                  isEditable={isString(obj[item]) && props.isEditable}
                  required={fieldIsRequired(item)}
